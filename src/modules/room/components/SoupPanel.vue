<template>
  <NCard class="h-full rounded-3xl border-0 shadow-soft" :content-style="{ padding: '16px 18px' }">
    <div class="grid h-full min-h-0 gap-4 xl:grid-rows-[auto_auto_minmax(0,1fr)]">
      <div class="rounded-[24px] bg-slate-900 px-5 py-5 text-white">
        <div class="flex flex-wrap items-center gap-2">
          <NTag round type="warning">{{ soupTitle }}</NTag>
          <NTag round type="success">{{ phaseLabel }}</NTag>
        </div>
        <div class="mt-4 text-xs uppercase tracking-[0.22em] text-slate-300">当前题面</div>
        <div class="mt-2 max-h-40 overflow-y-auto text-xl font-semibold leading-8">{{ prompt }}</div>
      ...
      <div class="grid gap-3 md:grid-cols-4">
        ...
        >
          ...
          <div class="mt-1 text-lg font-semibold text-slate-900">{{ panel.value }}</div>
          <div class="mt-1 text-xs text-slate-500">{{ panel.caption }}</div>
        </div>
      </div>
