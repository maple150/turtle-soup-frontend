<template>
  <NCard class="rounded-3xl border-0 shadow-soft" :content-style="{ padding: '12px 14px' }">
    <div class="grid gap-2.5">
      <div class="rounded-[20px] bg-slate-900 px-4 py-3.5 text-white">
        <div class="flex flex-wrap items-center gap-2">
          <NTag round type="warning">{{ soupTitle }}</NTag>
          <NTag round type="success">{{ phaseLabel }}</NTag>
        </div>
        <div class="mt-2.5 text-[11px] uppercase tracking-[0.22em] text-slate-300">当前题面</div>
        <div class="mt-1 max-h-24 overflow-y-auto text-base font-semibold leading-6">{{ prompt }}</div>
        <div class="mt-2 text-sm leading-5 text-slate-300">
          {{ hostHint }}
        </div>
      </div>
      ...
      <div class="grid gap-2 md:grid-cols-4">
        ...
        >
          ...
          <div class="mt-0.5 text-sm font-semibold text-slate-900">{{ panel.value }}</div>
          <div class="mt-0.5 text-[11px] text-slate-500">{{ panel.caption }}</div>
        </div>
      </div>

      <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm leading-5 text-slate-600">
